...
type UserService interface {
	All() ([]resp.UserResp, error)
	List(page req.PageReq, listReq req.UserQueryReq) (response.PageResp, error)
...
func (iSrv iUserService) All() ([]resp.UserResp, error) {
	var users []models.User
	if err := iSrv.db.Order("id desc").Find(&users).Error; err != nil {
		return nil, err
	}
	var res []resp.UserResp
	response.Copy(&resps, &users)
	return res, nil
}

func (iSrv iUserService) List(page req.PageReq, listReq req.UserQueryReq) (response.PageResp, error) {
	limit := page.PageSize
